...
// FIFO queues for orders, deliveries, couriers (maps to user's pseudocode)
...
export async function enqCustomer(order){
  // wrapper to insert order into Supabase and refresh queue
  try{ const sb = await getSupabase(); const { data, error } = await sb.from('orders').insert([order]).select(); if (error) throw error; await refreshOrders(); return data; }catch(e){ throw e; }
}
export async function deqCustomer(){
  // wrapper to dequeue (process) the oldest order — delete oldest row
  try{ const sb = await getSupabase(); const { data, error } = await sb.from('orders').select('*').order('created',{ascending:true}).limit(1).maybeSingle(); if (error) throw error; if (!data) return null; const del = await sb.from('orders').delete().eq('id', data.id); if (del.error) throw del.error; await refreshOrders(); return data; }catch(e){ throw e; }
}
...
export async function enqDelivery(delivery){ try{ const sb = await getSupabase(); const { data, error } = await sb.from('deliveries').insert([delivery]).select(); if (error) throw error; await refreshDeliveries(); return data; }catch(e){ throw e; } }
export async function deqDelivery(){ try{ const sb = await getSupabase(); const { data, error } = await sb.from('deliveries').select('*').order('created',{ascending:true}).limit(1).maybeSingle(); if (error) throw error; if (!data) return null; const del = await sb.from('deliveries').delete().eq('id', data.id); if (del.error) throw del.error; await refreshDeliveries(); return data; }catch(e){ throw e; } }
...
export async function enqCourier(courier){ try{ const sb = await getSupabase(); const { data, error } = await sb.from('couriers').insert([courier]).select(); if (error) throw error; await refreshCouriers(); return data; }catch(e){ throw e; } }
export async function deqCourier(){ try{ const sb = await getSupabase(); const { data, error } = await sb.from('couriers').select('*').order('created',{ascending:true}).limit(1).maybeSingle(); if (error) throw error; if (!data) return null; const del = await sb.from('couriers').delete().eq('id', data.id); if (del.error) throw del.error; await refreshCouriers(); return data; }catch(e){ throw e; } }
...
export async function pushComment(comment){ try{ const sb = await getSupabase(); const { data, error } = await sb.from('comments').insert([comment]).select(); if (error) throw error; await refreshComments(); return data; }catch(e){ throw e; } }
export async function popComment(){ try{ const sb = await getSupabase(); const { data, error } = await sb.from('comments').select('*').order('created_at',{ascending:false}).limit(1).maybeSingle(); if (error) throw error; if (!data) return null; const del = await sb.from('comments').delete().eq('id', data.id); if (del.error) throw del.error; await refreshComments(); return data; }catch(e){ throw e; } }
export async function listComments(){ try{ const sb = await getSupabase(); const { data, error } = await sb.from('comments').select('*').order('created_at',{ascending:false}); if (error) throw error; return data||[]; }catch(e){ throw e; } }
